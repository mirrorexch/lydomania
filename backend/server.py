...
async def lifespan(app: FastAPI):
    ...
    # Phase 7a — crash engine
    from services.crash import engine as _crash_engine
    await _crash_engine.start()
    _background_tasks.append(_crash_engine._loop_task)
    ...
    # Phase 7c — Battle Pass / Seasons cron (every 5 min: rollover if past ends_at)
    from services.season import ensure_indexes as _season_indexes, rollover_if_needed, get_or_create_active_season  # noqa: E402
    await _season_indexes()
    # Phase 8 — missions + achievements indexes
    try:
        from services.missions import ensure_indexes as _mi_indexes
        from services.achievements import ensure_indexes as _ach_indexes
        from services.marketplace import ensure_indexes as _mk_indexes
        from services.vip import ensure_indexes as _vip_indexes
        await _mi_indexes()
        await _ach_indexes()
        await _mk_indexes()
        await _vip_indexes()
    except Exception as _e:  # noqa: BLE001
        logger.warning("[phase8/9] index init failed: %s", _e)
    # Lazy-bootstrap the first season on startup so /api/season/current works
    try:
        bootstrap = await get_or_create_active_season()
        logger.info("[season] active=%s ends_at=%s", bootstrap["season_id"], bootstrap["ends_at"])
    except Exception as e:  # noqa: BLE001
        logger.warning("[season] bootstrap failed: %s", e)
    _scheduler.add_job(
        rollover_if_needed,
...
# Phase 7a — Crash (Rocket)
from routers.crash import router as crash_router  # noqa: E402
from routers.ws_crash import router as crash_ws_router  # noqa: E402
app.include_router(crash_router)
app.include_router(crash_ws_router)
# Phase 7b — Wheel of Fortune
from routers.wheel import router as wheel_router  # noqa: E402
app.include_router(wheel_router)
# Phase 7c — Battle Pass / Seasons
from routers.season import router as season_router  # noqa: E402
app.include_router(season_router)
# Phase 8 — Plinko + Mines + Missions + Activity + Achievements
from routers.plinko import router as plinko_router          # noqa: E402
from routers.mines import router as mines_router            # noqa: E402
...
from routers.vip import router as vip_router                  # noqa: E402
