...
export default function RoulettePage({ user, refreshBalance }) {
    ...
    return (
        <main className="mx-auto px-3 sm:px-6 pt-4 pb-24 lg:pb-6 space-y-4 max-w-[430px] sm:max-w-[640px] lg:max-w-[960px] xl:max-w-[1100px]"
            ...
            {/* Phase 6h — Hero banner: spinning roulette wheel + floating gifts.
                Phase 6i: fit full subject vertically — wheel never crops. */}
            <header
                data-testid="roulette-hero"
                className="relative overflow-hidden rounded-3xl border border-white/10 -mx-1"
                style={{
                    backgroundImage: "url(/banners/roulette.png)",
            ...
            >
                ...
                <div className="relative flex items-start justify-between gap-3 p-4 sm:p-5">
                    <button
                        ...
                        className="text-[10px] sm:text-xs flex items-center gap-1.5 bg-black/40 backdrop-blur border border-white/15 hover:border-gold-bright/55 rounded-full px-3 py-1.5 transition order-2"
                    >
                    ...
                    <div className="min-w-0 order-1 self-end">
                        <div className="text-[10px] uppercase tracking-[0.32em] text-gold-bright font-bold drop-shadow-[0_1px_4px_rgba(0,0,0,0.85)]">
                            {t("roulette.tag")}
                        </div>
            ...
            {/* Phase 6h — Live phase status chip moved out of the hero into its
                own row so the banner stays clean. */}
            <div className="flex items-center justify-between gap-2 px-1">
                <div
                    data-testid="roulette-phase-label"
                    className="text-xs sm:text-sm font-bold text-white/85 tabular-nums"
                >
                    {phaseLabel}
                </div>
            ...
            <div className="flex items-center gap-2 flex-wrap" data-testid="roulette-tier-pills">
                ...
                {TIERS.map((c) => {
                    ...
                    return (
                        <button
                            key={c}
                            onClick={() => { selectionChanged(); setBetTier(c); }}
                            data-testid={`tier-${c}`}
                            className={`px-4 py-1.5 rounded-full text-sm font-bold border transition tabular-nums ${
                                active
                                    ? "bg-gold-bright/20 text-gold-bright border-gold-bright/55 shadow-[0_0_15px_rgba(255,215,0,0.45)]"
                    ...
                    >
                        {activeBasket.items.map((it) => (
                            <GiftCard
                                key={it.item_slug}
                                item={{
                                    id: it.item_slug, item_name: it.item_name, item_slug: it.item_slug,
                                    image_url: it.image_url, rarity: it.rarity || "common",
                                    payout_ton: it.floor_ton,
                                }}
                                size="sm"
                                showLabel={false}
                                priceChip={`${formatTON(it.floor_ton)} TON`}
                                actionSlot={
                                    <span className="block text-center text-[9px] text-gold-200/60 font-bold tabular-nums">{it.draw_pct}%</span>
                                }
                                className="!w-full"
                                testid={`roulette-gift-${it.item_slug}`}
                            />
                        ))}
                    </div>
                </section>
                ...
                <div data-testid="roulette-bets-feed" className="space-y-1.5 max-h-[280px] overflow-y-auto pr-1">
                    ...
                    {bets.map((b) => (
                        <div key={b.bet_id} className="flex items-center gap-2 text-xs">
                            ...
                            {b.winning_item_slug ? (
                                <span className="text-gold-bright font-bold text-[10px] truncate max-w-[120px]">
                ...
                {reveal && (
                    ...
                    >
                        <motion.div
                            ...
                            className="relative w-full sm:max-w-sm bg-cyber-surface border border-gold-bright/45 rounded-3xl overflow-hidden text-center shadow-[0_0_60px_rgba(255,215,0,0.45)]"
                        >
                            <button
                                onClick={() => setReveal(null)}
                                data-testid="reveal-close"
                                className="absolute top-3 right-3 z-10 bg-cyber-bg/80 backdrop-blur rounded-full p-1.5 text-white/70 hover:text-white"
                                aria-label="close"
                            ><X className="w-4 h-4" /></button>
                            {/* Phase 6g — flush hero artwork fills the top of the modal */}
                            <div className="relative aspect-square bg-cyber-bg">
                                ...
                                />
                                <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-cyber-surface pointer-events-none" />
                                <div className="absolute top-3 left-3 text-[10px] uppercase tracking-[0.3em] text-gold-bright font-black bg-cyber-bg/80 backdrop-blur rounded-md px-2 py-1 border border-gold-bright/45">
                                    {t("roulette.reveal.won_tag")}
                                </div>
                            </div>
                            <div className="p-5 pt-3">
                                <div className="font-display text-2xl font-black mb-1">{reveal.item_name}</div>
                                <div className="text-gold-bright font-bold text-lg mb-4 tabular-nums">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    ...
                                    <button
                                        ...
                                        disabled={revealBusy}
                                        className="rounded-xl py-3 bg-gradient-to-b from-gold-300 to-gold-500 hover:brightness-110 text-zinc-950 font-bold uppercase tracking-wider text-xs disabled:opacity-60 inline-flex items-center justify-center gap-1 shadow-[0_8px_24px_-6px_rgba(212,175,55,0.55)]"
                                    >
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-end sm:items-center justify-center p-3 backdrop-blur-sm"
            ...
            <motion.div
                ...
                onClick={(e) => e.stopPropagation()}
                className="bg-cyber-surface border border-gold-500/30 rounded-2xl max-w-lg w-full p-5 max-h-[85vh] overflow-y-auto"
            >
