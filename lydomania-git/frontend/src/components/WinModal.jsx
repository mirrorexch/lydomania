...
import { tapMedium, tapHeavy, tapForRarity, notifySuccess, notifyError } from "@/lib/haptics";

function burst(rarity) {
    const tier = rarityRank(rarity);
    const colors =
        rarity === "jackpot"
            ? ["#FFD700", "#8A2BE2", "#FF003C", "#FFB800", "#FF00E5"]
            : rarity === "mythic"
            ? ["#FF003C", "#FF00E5", "#8A2BE2"]
            : rarity === "legendary"
            ? ["#FFB800", "#FF8800", "#FFD700"]
            : rarity === "epic"
            ? ["#8A2BE2", "#FFD700"]
            : rarity === "rare"
            ? ["#FFD700", "#FFFFFF"]
...
export const WinModal = ({ open, roll, casePrice, onSell, onKeep, onClose, busy = false }) => {
    ...
    useEffect(() => {
        if (open && item) {
            ...
            sfx.playWin(rarity);
            tapForRarity(rarity);
            // Phase 11.1 — Layer gold-only legendary burst on top per user spec:
            // Common/Rare → none, Epic → normal, Legendary+ → epic intensity.
            if (rarity === "epic") {
                setTimeout(() => fireLegendaryBurst({ intensity: "normal" }), 80);
            } else if (rarity === "legendary" || rarity === "mythic" || rarity === "jackpot") {
                setTimeout(() => fireLegendaryBurst({ intensity: "epic" }), 80);
            }
            // Phase 6h — Jackpot / mythic celebration: third burst ring +
            // sparkle layer at 1.6s for an extended hold (no auto-dismiss
            // here — modal stays until user taps SELL / KEEP / outside).
            if (multiplier >= 5) {
                setTimeout(() => burst(rarity), 600);
                setTimeout(() => sfx.play("coin_drop", { volume: 0.7 }), 800);
            }
            if (rarity === "mythic" || rarity === "jackpot" || multiplier >= 10) {
                setTimeout(() => burst(rarity), 1100);
                setTimeout(() => burst(rarity), 1700);
                setTimeout(() => sfx.play("confetti_burst", { volume: 0.55 }), 1300);
                tapHeavy();
            }
        }
                    ...
                    >
                        ...
                        <div className="relative text-center">
                            ...
                            <motion.div
                                ...
                                className="mt-4 mx-auto relative aspect-square w-full max-w-[260px] rounded-2xl overflow-hidden bg-cyber-bg"
                            ...
                            <div className="mt-5 flex gap-2">
                                <button
                                    ...
                                    onClick={() => { tapMedium(); onSell(roll.inventory_id); }}
                                    disabled={busy}
