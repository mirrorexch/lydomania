...
async def _process_tx(tx: dict) -> None:
    ...
    if credited:
        ...
        if u:
            ...
            # Phase 7c — Battle Pass: +50 XP to referrer on user's FIRST credited deposit ≥ 1 TON.
            # Idempotent via the referee user_id (one award per referee, ever).
            try:
                if amount_ton >= 1.0 and u.get("referred_by_user_id"):
                    prior = await deposits_col.count_documents(
                        {"user_id": user_id, "credited": True, "tx_hash": {"$ne": tx_hash}},
                    )
                    if prior == 0:
                        from services.season import award_xp as _award_xp
                        await _award_xp(
                            u["referred_by_user_id"], 50,
                            "referral_first_deposit", f"ref_first:{user_id}",
                        )
            except Exception as _e:  # noqa: BLE001
                logger.warning("deposit_watcher: referral XP hook failed user=%s: %s", user_id, _e)
    else:
