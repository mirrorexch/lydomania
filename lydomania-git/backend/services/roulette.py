            ...
            if ROULETTE_PRIZE_MODE == "gifts":
                ...
                if not basket or not basket.get("items"):
                    ...
                    if upd_bet:
                        ...
                        # Phase 7c — +2 XP per TON wagered (gift-mode fallback path)
                        try:
                            from services.season import award_xp as _award_xp
                            await _award_xp(
                                bet["user_id"], int(float(bet["amount_ton"]) * 2),
                                "roulette_win", bet["bet_id"],
                            )
                        except Exception as _e:  # noqa: BLE001
                            LOG.warning("roulette: season XP hook failed bet_id=%s: %s", bet["bet_id"], _e)
                    continue
                ...
                if upd_bet:
                    ...
                    # Phase 7c — +2 XP per TON wagered on winning bet (gift mode)
                    try:
                        from services.season import award_xp as _award_xp
                        await _award_xp(
                            bet["user_id"], int(float(bet["amount_ton"]) * 2),
                ...
                if upd_bet:
                    ...
                    # Phase 7c — +2 XP per TON wagered (legacy TON-payout path)
                    try:
                        from services.season import award_xp as _award_xp
                        await _award_xp(
                            bet["user_id"], int(float(bet["amount_ton"]) * 2),
