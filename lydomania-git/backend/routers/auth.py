...
async def me(user: dict = Depends(get_current_user)) -> UserOut:
    # Phase 7c — Battle Pass: at-most-once-per-UTC-date login XP. Errors are silent.
    try:
        from services.season import maybe_award_daily_login as _daily_login
        await _daily_login(user["id"])
