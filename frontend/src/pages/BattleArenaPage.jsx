        ...
        <main className="mx-auto px-3 sm:px-6 pt-3 pb-24 lg:pb-6 space-y-4 max-w-[430px] sm:max-w-[900px] lg:max-w-[1300px]"
            ...
            <header className="flex items-center justify-between gap-2 sticky top-[52px] lg:top-0 z-20 -mx-1 px-1 py-2 bg-cyber-bg/90 backdrop-blur-xl">
                ...
                <div className="text-center">
                    <div className="text-[10px] uppercase tracking-[0.3em] text-amber-400 font-bold flex items-center justify-center gap-1">
                    ...
                    <div data-testid="battle-status" className="text-sm font-display font-extrabold">
                        ...
                        {battle.status === "ready" && countdown && (
                            <span className="ml-2 text-yellow-300 tabular-nums">{countdown.secLeft}s</span>
                        )}
                    </div>
                </div>
                <button onClick={openVerifier} data-testid="battle-pf-btn"
                    ...
                    <Shield className="w-3 h-3 text-cyan-300" />{t("battles.pf_short")}
                </button>
            </header>
            ...
            <div className="flex items-center justify-between rounded-lg border border-white/10 bg-white/[0.03] p-3 text-xs">
                <div className="flex items-center gap-3">
                    ...
                    <span className="font-mono text-cyan-200 tabular-nums">{formatTON(battle.pot_ton)} TON</span>
    ...
    return (
        <div
            ...
            className={`rounded-2xl border bg-cyber-surface/70 p-3 space-y-3 ${
                won ? "border-amber-400/60 shadow-[0_0_30px_rgba(251,191,36,0.4)]"
                    : isYou ? "border-cyan-400/40"
                    : "border-white/10"
            }`}
        >
                ...
                <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                        <div className="text-xs font-bold truncate">
                            ...
                            {isYou && <span className="ml-1 text-cyan-300 text-[10px]">({t("battles.you")})</span>}
                        </div>
                        {won && <div className="text-[10px] text-amber-300 font-bold flex items-center gap-1"><Trophy className="w-3 h-3" />{t("battles.winner")}</div>}
                    </div>
                </div>
                <div className="text-right">
                    <div className="text-[9px] uppercase tracking-wider text-white/40">{t("battles.total")}</div>
                    <div className={`font-display font-bold text-base tabular-nums ${won ? "text-amber-300" : "text-cyan-200"}`}>
            ...
            <div className="h-[78px] rounded-lg bg-black/30 border border-white/10 relative overflow-hidden">
                {activeReveal && activeReveal.item ? (
                    ...
                    >
                        ...
                        <div className="ml-3">
                            <div className="text-[11px] font-bold text-white truncate max-w-[140px]">{activeReveal.item.name}</div>
                            <div className="text-[10px] text-emerald-300 tabular-nums">+{formatTON(activeReveal.payout_ton)} TON</div>
                        </div>
                    </motion.div>
                ) : (
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1" data-testid={`seat-${seat.seat_index}-reveals`}>
                {reveals.map((r, idx) => (
                    <div key={idx} className="flex-shrink-0 w-12 h-12 rounded-md bg-white/[0.05] border border-white/10 flex items-center justify-center relative">
                        {r.item?.image_path ? (
                            <img src={resolveImage(`/api/static/${r.item.image_path}`)}
                                 onError={(e) => { e.target.src = resolveImage("/api/static/cases/crate_common.png"); }}
                                 className="w-10 h-10 object-cover rounded" />
                        ) : (
                            <span className="text-[8px] font-mono text-white/40">{r.item_slug?.slice(0, 6)}</span>
                        )}
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-end sm:items-center justify-center p-3 backdrop-blur-sm"
            ...
            <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
                ...
                className="bg-cyber-surface border border-amber-400/30 rounded-2xl max-w-xl w-full p-5 max-h-[85vh] overflow-y-auto"
            >
